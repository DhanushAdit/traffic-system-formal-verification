...
def _is_right_turn(prev: CarState, nxt: CarState) -> bool:
    info = _crossing_info(prev, nxt)
    if info is None:
        return False
    return nxt.edge.dir() == _RIGHT_TURN_MAP[prev.edge.dir()]
...
def evaluate_checks(
    ...
    # Right turns are also illegal (project spec: "cannot make right turn")
    for car_id, prev in prev_cars.items():
        nxt_car = next_cars.get(car_id)
        if nxt_car is None:
            continue
        if _is_right_turn(prev, nxt_car):
            illegal_direction_count += 1
